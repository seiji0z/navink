...
const StepUploadPreview = ({ onNext }) => {
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto bg-white rounded-2xl shadow-md p-4 sm:p-6">
      {loading && (
  ...
  <h2 className="text-xl sm:text-2xl font-semibold text-sky-700 mb-4 sm:mb-6">Upload and Preview File</h2>

  <div className="flex flex-col items-center border-2 border-dashed border-sky-300 rounded-xl p-10 sm:p-16 md:p-20 hover:bg-sky-50 transition">
        ...
        {file ? (
          <>
            <p className="mt-3 text-gray-700 text-center">
              Selected: <span className="font-semibold" title={file.name}>{file.name}</span>
            </p>
            {classification && (
      ...
      {previewUrl && (
        <div className="mt-6 border rounded-lg overflow-hidden">
          {file.type.startsWith("image/") ? (
            ...
            <iframe src={previewUrl} title="File Preview" className="w-full h-72 sm:h-80" />
          )}
        </div>
      )}

      <div className="flex flex-col sm:flex-row sm:justify-end sm:space-x-3 gap-3 mt-6">
        <button onClick={handleUpload} disabled={!file || loading} className={`w-full sm:w-auto px-6 py-2 rounded-lg font-medium transition ${file && !loading ? "bg-sky-500 text-white hover:bg-sky-600" : "bg-gray-300 text-gray-600 cursor-not-allowed"}`}>
          Upload & Continue
        </button>
      </div>
    </div>
